...
public class MainActivity extends AppCompatActivity {
        ...
        testService.get().enqueue(new Callback<GetInfo>() {
            @Override public void onResponse(final Call<GetInfo> call, final Response<GetInfo> response) {
                ...
                for (int i = 0; i < videoList.size(); i++) {
                    final Video v = videoList.get(i);
                    list.add("student_id: " + v.getStudentId());
                    list.add("user_name: " + v.getUserName());
                    list.add("image_url: " + v.getImageUrl());
                    list.add("video_url: " + v.getVideoUrl());
                    list.add(" ");
                }
    ...
    private void uploadInfo() throws IOException {
        ...
        testService.post("119034910241", "Eric YANG", "salted fish", getMultipartFromAsset(), getMultipartFromAsset1())
